...
function renderPage1ExecutiveSnapshot(doc: PDFKit.PDFDocument, report: ReportRecord, theme: ReturnType<typeof buildPdfTheme>) {
  const { colors } = theme;
...
function renderPdf(report: ReportRecord): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    renderPage3AnswerAppendix(doc, theme, mappedAnswersByPillar);
